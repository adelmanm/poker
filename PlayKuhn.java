...
public class PlayKuhn  
{
	...
	static void play_round()
	{
		...
		while (!h.is_terminal()) {
			if (h.is_chance()) {
				...
				h = (HistoryNodeKuhn)do_player_turn(h);
			}
			else if (h.get_player() == 1-player){
				h = (HistoryNodeKuhn)do_computer_turn(h);
			}
		}
	...
	static void do_chance(History h) 
	{
		...
		int card = ((HistoryNodeKuhn)h).cards[player];
	...
	static void calculate_payoff(History h)
	{
		...
		if (!((HistoryNodeKuhn)h).decisions.endsWith("P")) {
			System.out.println("Computer card is " + ((HistoryNodeKuhn)h).cards[1-player]);
		}
